...
if (typeof window !== 'undefined') {
  currentAccessToken = safeStorage.getItem('accessToken');
  ...
  try {
    ...
    if (urlToken) {
      ...
      safeStorage.setItem('accessToken', urlToken);
    }
  } catch (e) {
    console.warn('[API] Failed to parse token from URL', e);
  }
}
...
export const setAccessToken = (token) => {
  ...
  if (typeof window !== 'undefined') {
    if (token) safeStorage.setItem('accessToken', token);
    else safeStorage.removeItem('accessToken');
  }
};
