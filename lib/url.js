...
export const downloadMedia = (fileUrl) => {
  if (!fileUrl) return;
  // Use our backend proxy to force download
  const downloadUrl = `${API_URL}/user/download?url=${encodeURIComponent(fileUrl)}`;
  
  // Create a temporary link to trigger download
  const link = document.createElement('a');
  link.href = downloadUrl;
  link.setAttribute('target', '_blank'); // Open in new tab to start the download
